...
export async function GET(request: Request): Promise<Response> {
  ...
  const filename = template
    ? "neatbynicky-menus-template.csv"
    : `neatbynicky-menus-${stamp}.csv`;
...
async function buildRows(): Promise<string[][]> {
  ...
  return rows
    .filter(
      (row) => row.category?.kind === "cafe" || row.category?.kind === "both",
    )
    .map((row) => [
      row.sku ?? "",
      row.name,
